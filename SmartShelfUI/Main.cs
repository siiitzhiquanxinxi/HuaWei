...
namespace SmartShelfUI
{
    public partial class Main : Form
    {
                            ...
                            if (this.OpenLockType == unlockTypeEnum.计划领料)
                            {
                                ...
                                foreach (KeyValuePair<string, int> item in dic_wait_for_open_shelf)
                                {
                                    if (item.Value == 0)
                                    {
                                        ...
                                        this.BeginInvoke((MethodInvoker)delegate
                                        {
                                            if (MessageBox.Show("检测到抽屉超时未关闭，是否继续打开下面的抽屉？", "是否继续？", MessageBoxButtons.OKCancel) == DialogResult.OK)
                                            {
                                                MySocket.SyncSend(code_byte);
                                            }
                                        });
        ...
        void showFormLogin()
        {
            ...
            frmLogin.sendRefresh += new ChildForm.Login.FormHandle(SendRefreshLock);
